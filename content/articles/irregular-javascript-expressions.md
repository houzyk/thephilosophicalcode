...
We can now to cash out the tension between JavaScript regular expressions and their theoretical counterpart. Notice that once we add memory (like an unbounded stack), we move up Chomsky's Hierarchy moving away from regular languages. So, any machine requiring memory recognises a larger class of languages than regular languages. In other words, any computation that relies on memory recognises a different set of languages than regular languages. Since JavaScript regular expressions rely on memory, just like context-free languages, they correspond to languages in the hierarchy that move away from regular languages. In other words, JavaScript regular expressions each act as syntatic sugar for a set of languages belonging to a different containment level in the hierarchy than regular languages. So, they recognise a larger and different class of languages than DFAs and regular languages. Hence, they are not regular.
