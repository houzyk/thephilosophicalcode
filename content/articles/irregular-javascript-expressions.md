---
external: false
title: "(Ir)regular JavaScript Expressions"
description: "The MDN reference on JavaScript regular expressions notes that \"JavaScript regular expressions are in fact not regular\". This highlights a subtle tension between actual regular expressions and their implementation in JavaScript. In this article, we examine this tension by contrasting JavaScript regular expressions with their theoretical counterpart."
author: "Muhammad Houzair Koussa"
authorUrl: "https://github.com/houzyk"
ogImagePath: "/images/irregular-javascript-expressions/cover.webp"
date: 2026-04-04
---

![(Ir)regular JavaScript Expressions](/images/irregular-javascript-expressions/cover.webp)

> [The MDN reference](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Regular_expressions "The MDN reference") on JavaScript regular expressions notes that "JavaScript regular expressions are in fact not regular". This highlights a subtle tension between actual regular expressions and their implementation in JavaScript. In this article, we examine this tension by contrasting JavaScript regular expressions with their theoretical counterpart.

## Abstract

For context, here's the full note from MDN:

> JavaScript regular expressions are in fact not regular, due to the existence of backreferences (regular expressions must have finite states). However, they are still a very useful feature.

By drawing on ideas from this note, this article is organised into three sections. Firstly, we explore the theory behind actual regular expressions to understand why they must have finite states. Secondly, we cash out the underlying tension (between actual and JavaScript regular expressions) as an issue of *language recognition*. Finally, we demonstrate the usefulness of backreferences and conceptually understand how they make JavaScript regular expressions irregular.

In section 1, we show how regular expressions act as syntactic sugar for a class of languages called 'regular languages'. We then discuss how these languages are recognised by and are intrinsically tied to a class of *finite state* machines called 'Deterministic Finite Automata' (DFA).

In section 2, we take a look at Chomsky's hierarchy. The latter hints at how different classes of languages are recognised by different classes of machines. This gives us a way to cash out the aforementioned tension by talking about the different levels in the hierarchy. Essentially, to understand why JavaScript regular expressions are not regular *is* to understand how they recognise a larger (hence different) class of languages than actual regular expressions.

In section 3, we dive into some code to demonstrate the usefulness of backreferences. We then conceptually analyse the way they work to abduct that they rely on memory. A priori, JavaScript regular expressions rely on memory. In contrast, DFAs are finite state machines without memory that are intrinsically tied to regular languages. So, JavaScript regular expressions are not regular because they recognise a different class of languages compared to DFAs.

## 1. The Theory Behind Regular Expressions

### 1.1 Regular Expressions As Syntactic Sugar

In order to build up the case for viewing regular exressions as syntactic sugar, let's consider two observations about them.

Firstly, regular expressions are governed by strict syntax rules. For example, the string `a{1` is an invalid regular expression because it's missing the `}`, whereas `a{1}` is valid. This shows that regular expressions are sentences of a formal language. For our purposes, we may say that a formal language is one with a finite alphabet and sufficiently strict syntax rules. Together, they determine how sentences in the language are formed. Essentially, a string is a sentece of a formal language if and only if it can be generated by the rules and alphabet of the language. So, a formal language is characteristically a set of strings following some strict syntax rules. In the case of regular expressions, we may define their formal language as follows.

The alphabat is a set of literal strings (like the set of all Unicode characters) including the empty string `''`. The syntax rules are then defined recursively.

1. The empty set is a regular expression [1].
2. The empty string `''` is a regular expression.
3. Any literal string from the alphabet is a regular expression.
4. For any two regular expressions `R` and `S`,
    1. The concatonation `RS` is a regular expression.
    2. The alternation `R|S` (the set union of all the strings in `R` and `S`) is a regular expression.
    3. The Kleene star `R*` (the set of all strings generated by concatonating any finite non-negative number of strings from `R`) is a regular expression.
5. Nothing else is a regular expression.


For example, from rule 3, we can say that `a` is a regular expression (which matches the string `'a'`) and that `b` is a regular expression (which matches the string `'b'`). Applying rule 4.1, we get `ab` as regular expression. Further applying rule 4.2, we get `ab|a` which is a regular expression matching strings like `'a'`, `'ab'` or `'b'`. 

As a side note, one may feel that these set of rules are incomplete because in popular programming languages regular expressions more operators than shown in our rules (like `+` to mean at least 1 or more). However, these other operators may be defined from our given set of rules. For example, we can define the `+` operator by only using as follows: 

`R+` = `RR*`

Here's a table of common operators and their definitions

| Operator | Name | Definition | Example |
|---|---|---|---|
| `R+` | One-or-more | `RR*` | `a+` = `aa*` |
| `R?` | Optional | `R\|''` | `a?` = `a\|''` |
| `R{n}` | Exactly n | `RR...R` (n concatenations) | `a{3}` = `aaa` |
| `R{n,m}` | Between n and m | `R{n}` concat `R?` repeated (m−n) times | `a{2,4}` = `aa(a\|'')(a\|'')` |
| `[abc]` | Character class | `a\|b\|c` | `[aeiou]` = `a\|e\|i\|o\|u` |
| `[^abc]` | Negated class | Alternation of all alphabet symbols *not* listed | `[^ab]` = `c\|d\|...` |
| `.` | Any character | Alternation of every symbol in the alphabet | `a.b` = `a(x\|y\|z\|...)b` |

Secondly, there are regular expressions that have more than one instance of a match (some even have infinite matches). Intuitively, we can form a set of all these possible matches of an arbitrary regular expression. So, when we say that a string matches a particular regular expression, what we're saying is that that string is in the set of strings . In other words, a regexp defines a regular language, a set of strings, and a match is an element of that set.

So far, we've observed that regular expressions obey strict syntactic rules which describe a set of strings. Intuitively, we may understand this as saying that regular expressions act as syntactic sugar for a class of languages. These laguages are commonly refered to as 'regular languages'.


### 1.2 Regular Languages

### 1.3 Deterministic Finite Automata (DFA)

## 2. An Issue Of Language Recognition

### 2.1 Chomsky's Hierarchy

### 2.2 Language Recognition

## 3. Backreferences

### 3.1 The usefulness of backreferences

### 3.2 Conceptual Analysis

### 3.3 Memory And Finite States

## Footnotes

1. 

2. ...

![Relation between regular expressions, DFAs, NFAs, regular languages and regular grammar](/images/irregular-javascript-expressions/relation_between_regexp_dfa_nfa_reg_lang_reg_gram.webp)