...
4. For any two regular expressions `R` and `S`,
    ...
    2. The alternation `R|S` is a regular expression.
5. For any regular expression `R`,
    1. The Kleene star `R*` is a regular expression.
    2. The grouping `(R)` is a regular expression. It is equivalent to `R`.
...
Informally, the general pumping lemma for regular languages states that, for any regular language `RL`, if a string `uwv` in `RL` contains a sufficiently long substring `w`, then `w` can be split into 3 parts `xyz` (where `y` must have a non-zero length). Then, we can keep on pumping up the number of `y`'s such that `uxy...yzv` remains in `RL` [3]. 
...
For example, take the string `'aaaa'` from the regular language of even-length strings over an alphabet like `{'a'}`.
...
We can now pump up the number of `y`'s. Let's pump it 2 times. Since `aᵏ` contains at least one `a` (and only `a`'s) and `y` must have a non-zero length, then `y` must also contain at least one `a` and only `a`'s.
